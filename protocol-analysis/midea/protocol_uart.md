...
This document is the byte-level reference for the Midea UART serial protocol used
between the Wi-Fi module (CN3 connector) and the mainboard inside Midea residential
air conditioning indoor units. It covers framing, integrity checks, all known
command and response types, and notification messages.

No official Midea protocol specification is publicly available. This reference is
assembled from open-source reverse-engineering projects, community forum discussions,
...
| `mill1000/midea-msmart` | Python | High | Fork with advanced documentation; see `midea-msmart-mill1000.md` for extracted findings (Findings 1-14) |
| `mill1000/midea-msmart` Finding 13 | — | High | UART protocol reference: MSG_TYPE behavior, boot sequence, body handling (see `midea-msmart-mill1000.md`) |
...
### 4.1 Message Types (byte 9 of frame)

| msg_type | Direction | Purpose | Confidence |
|----------|-----------|---------|------------|
| `0x02` | Dongle → Appliance | Command (set): body[0]=0x40 (set status), 0xB0 (property control) | Confirmed |
| `0x03` | Appliance → Dongle | Response: body[0]=0xC0 (status), 0xC1 (extended), 0xB1 (property), 0xB5 (capabilities) | Confirmed |
| `0x04` | Appliance → Dongle | Heartbeats: body[0]=0xA1 (energy/temp), 0xA2-0xA6 (device params) | Confirmed |
| `0x05` | Appliance → Dongle | Handshake / ACK: body[0]=0xA0 | Confirmed |
| `0x06` | Appliance → Dongle | Status upload (same handler as 0x04, forwarded to cloud as packet type 0x40) | Consistent |
| `0x07` | Bidirectional | Device identification (SN + appliance type) | Confirmed |
| `0x0A` | Appliance → Dongle | Error report (forwarded to cloud as packet type 0x44) | Consistent |
| `0x0D` | Dongle → Appliance | Network init / SoftAP init (boot or WiFi pairing) | Confirmed |
| `0x0F` | Appliance → Dongle | Status transport (forwarded to cloud as packet type 0x20) | Consistent |
| `0x11` | Appliance → Dongle | Status transport (same handler as 0x0F) | Consistent |
| `0x13` | Appliance → Dongle | Config data (dongle reads 6 bytes, stores locally) | Consistent |
| `0x14` | Appliance → Dongle | Accepted (range pair with 0x15) | Consistent |
| `0x15` | Appliance → Dongle | Accepted (range pair with 0x14, triggers reboot) | Consistent |
| `0x16` | Dongle → Appliance | Device event (4-byte event data in body) | Consistent |
| `0x61` | Dongle → Appliance | Time sync | Consistent |
| `0x63` | Dongle → Appliance | Network status report (SSID, signal, IP, connection status) | Confirmed |
| `0x64` | Appliance → Dongle | OTA / key trigger (initiates cloud key log upload or OTA) | Consistent |
| `0x65` | Bidirectional | RAC serial number query | Confirmed |
| `0x68` | Appliance → Dongle | WiFi config (SSID/password) | Consistent |
| `0x6B` | Appliance → Dongle | Passthrough (raw frame forwarded) | Consistent |
| `0x81` | Appliance → Dongle | Ignored (received but not processed) | Consistent |
| `0x82` | Appliance → Dongle | Mode check (protocol mode verification) | Consistent |
| `0x83` | Appliance → Dongle | Config reset (clears 265 bytes of config) | Consistent |
| `0x84` | Appliance → Dongle | Ignored (received but not processed) | Consistent |
| `0x85` | Appliance → Dongle | Config response (sends confirmation back) | Consistent |
| `0x87` | Appliance → Dongle | Fixed response (version info) | Consistent |
| `0x90` | Appliance → Dongle | Exception (rejected if LEN <= 0x0B) | Consistent |
| `0x9A` | Dongle → Cloud | WiFi version report (NOT over UART — direct to cloud, every 15 min) | Consistent |

Source: mill1000/midea-msmart Finding 13 (see `midea-msmart-mill1000.md`). The 8 types
marked **Confirmed** are verified in own captures; the remaining 20 types are
**Consistent** (single-source, not yet captured on hardware).
...
### 4.2 Response Dispatch (msg_type + body[0])

```
msg_type 0x02 + body[0] 0xC0 → Status response (§6.1)
msg_type 0x03 + body[0] 0xB1 → Property response (§5.5)
msg_type 0x03 + body[0] 0xB5 → Capabilities response (§5.4)
msg_type 0x03 + body[0] 0xC0 → Status notification (§6.1)
msg_type 0x03 + body[0] 0xC1 → Extended response (§6.2)
msg_type 0x04 + body[0] 0xA0 → Heartbeat ACK (§7.3)
msg_type 0x04 + body[0] 0xA1 → Heartbeat energy/temps (§7.1)
msg_type 0x04 + body[0] 0xA2 → Heartbeat device params (§7.3)
msg_type 0x04 + body[0] 0xA3 → Heartbeat device params 2 (§7.3)
msg_type 0x04 + body[0] 0xA5 → Heartbeat outdoor unit (§7.3)
msg_type 0x04 + body[0] 0xA6 → Heartbeat network info (§7.3)
msg_type 0x05 + body[0] 0xA0 → Handshake ACK (§8)
msg_type 0x07                → Device identification (§7.5)
msg_type 0x63                → Network status request (§7.4)
msg_type 0x65                → RAC serial number (§7.6)
```

---

## 5. Commands (Dongle → Appliance)

### 5.1 Command 0x41 — Query Variants
...
#### 5.1.1 Status query (body[1]=0x81, body[2]=0x00)
...
#### 5.1.2 Capability page query (body[1]=0x81, body[2]=0x01) — Group-Based Dev Params
...
**Cross-reference**: The A1 heartbeat (§7.2) also carries `curWorkedDay/Hour/Min` at
...
Extended State sub-page 0x02 (§6.2.4) body[26]-[29]. Same data, different query path.
...
**Cross-reference**: Vane angle fields overlap with Extended State sub-page 0x02 (§6.2.4)
...
#### 5.1.3 Power usage query (body[1]=0x21, body[2]=0x01, body[3]=0x44)

Triggers a `0xC1` power BCD response. See §6.2.1 for response parsing.
...
#### 5.1.4 Extended query (body[1]=0x21, body[2] varies, optCommand in body[4])
...
| 0x02 | Extended state query | 0xC1 sub-pages 0x01 + 0x02 (see §6.2.3, §6.2.4) |
...
#### 5.1.5 Display toggle (body[1]=0x61)
...
#### 5.1.6 Lightweight parameter query (CMDTYPE_QUERY_PAR)
...
#### 6.2.2 Group Page Response — R/T Bus Group-Based Dev Params (body[1]=0x21, body[2]=0x01)
...
Finding 11). Field-level documentation for each observed page is in **§5.1.2**
...
#### 6.2.3 Extended State Sub-page 0x01 — Sensor Temperatures, Fault Flags, Operating State
...
**Cross-references to Group Pages (§5.1.2)**:
...
(0.5 C steps). The 0xC0 status response (§6.1) uses yet another encoding: `(raw - 50) / 2` for
both indoor and outdoor. Three different encodings for the same physical sensors.

---

#### 6.2.4 Extended State Sub-page 0x02 — Status Flags, Timers, Power, Vane Angles, Compressor
...
| Aspect | Group Pages (§5.1.2) | Extended State (§6.2.3/§6.2.4) |
|--------|----------------------|--------------------------------|
...
### 5.3 Command 0x93 — Extension Board / Wall Controller Status
...
### 5.4 Command 0xB5 — Capabilities Query and Response
...
#### 5.4.1 New Protocol B5 (TLV format, msg_type=0x03)
...
#### 5.4.2 Old Protocol B5 (Fixed format, msg_type=0x05)
...
### 5.5 Command 0xB0/0xB1 — Property Protocol (newer devices)
...
| 0x1A, 0x00 | tone / buzzer | 1 | 0x00=off, 0x01=on. Cross-ref: §5.1.4 buzzer mechanism #3 |
...
## 6. Responses (Appliance → Dongle)

### 6.1 Response 0xC0 — Current AC Status
...
the remote/phone temperature as the room reference instead of its own sensor. See §5.1.4
...
The capability flag `SILKY_COOL` (0x0018) in the B5 response (§5.4) indicates whether the
...
### 6.2 Response 0xC1 — Extended (Group Pages, Power, Extended State)
...
#### 6.2.1 Power Usage Response — BCD kWh (body[3]=0x44)
...
## 7. Notifications (Mainboard → Dongle, unsolicited)
...
### 7.2 Heartbeat 0xA1 — Energy and Temperatures (confirmed)
...
### 7.5 Network Status (msg_type=0x63 / 0x0D)

- **0x63**: AC polls dongle for network status. Dongle responds with a 20-byte body.
- **0x0D**: Network initialization — sent by dongle at boot (see §8.1).
...
### 7.6 Device Identification (msg_type=0x07)
...
### 7.8 0xBB Sub-Protocol (SN8 / newer devices)
...
## 8. Handshake and Initialization
...
5. **Capabilities query** (CMD 0xB5): Dongle queries device capabilities.
   B5 response determines which features are available (see §5.4).
...
|----|-------|---------|--------|-------|
| OQ-01 | ~~PMV mode value meanings~~ | §6.1 | **Resolved** | Full PMV table added (-3.0 to +2.5, 13 values) from node-mideahvac + Lua Q1B |
| OQ-02 | Alternative temp packing trigger | §6.1 | Hypothesis | Trigger condition for byte[1] vs byte[2] format unknown |
| OQ-03 | ~~Silky cool frame-length conditional~~ | §6.1 | **Resolved** | Documented: body[22] only present if body >= 23 bytes; B5 SILKY_COOL (0x0018) capability flag |
| OQ-04 | A2/A3/A5/A6 heartbeat body structures | §7.4 | Unknown | Present in captures, no source decodes them |
| OQ-05 | 0xBB sub-protocol (SN8 / newer units) | §7.8 | Not Covered | Entirely different encoding; midea-local is primary ref |
| OQ-06 | ECO bit read/write position | §6.1 / §5.2 | Documented | Response bit 4 vs command bit 7 — confirmed intentionally different positions (Finding 4) |
| OQ-07 | Display ON bit inversion | §6.1 | Disputed | This doc says bits==0x7=ON; midea-local says bits!=0x7=ON. Needs hardware test |
| OQ-08 | Group 2 captures vs Finding 11 field mismatch | §5.1.2 | Disputed | Own R/T captures show temps where Finding 11 expects RPM |
| OQ-09 | Extension board 0x93 field meanings | §5.3 | Unknown | All payload fields unidentified |
| OQ-10 | C1 power subbody type 0x40 | §6.2.1 | Not Covered | Only 0x44 format documented; 0x40 exists but undocumented |
| OQ-11 | Display toggle body[1] variant | §5.1.5 | Variant | dudanov=0x61, midea-local=0x02/0x42; device generation dependent |
| OQ-12 | ~~Section 5.1.4 extended query~~ | §5.1.4 | **Resolved** | Fully expanded: frame layout, optCommand table, Follow Me encoding, queryStat values |
| OQ-13 | ~~B0/B1 property protocol~~ | §5.5 | **Resolved** | Expanded to 53 property IDs from Lua Q14 validation |
| OQ-14 | Group 0 byte order (16-bit BE vs LE) | §5.1.2 | Unknown | Group 0 uses 16-bit BE for days; Group 5 uses 16-bit LE — inconsistency needs verification |
| OQ-15 | Old B5 fixed-format field details | §5.4.2 | Hypothesis | Cursor-based field map from single source; not verified |
| OQ-16 | Follow Me body[8] bit 7 — Lua vs Python/weex.js | §5.1.4 | Disputed | Python/weex.js put bodySense at body[8] bit 7. Lua SET command has strongWind/comfortableSleep/power_saving at body[8] instead. Needs hardware test |
| OQ-17 | localBodySense not found | §5.1.4 | Unknown | Finding 10b claims body[9] bit 7 = localBodySense, but not found in weex.js or Python |
| OQ-18 | 0xA0 alternative temp encoding | §7.3 | Hypothesis | weex.js `praserA0` uses `((info[1] & 0x3E) >> 1) + 12` with half-degree in bit 6. Trigger: dataType=0x05, body[0]=0xA0 |
| OQ-19 | 20 new MSG_TYPEs unverified | §4.1 | Consistent | Firmware-verified but not captured on own hardware |
| OQ-20 | Python msmart: no A1 heartbeat | — | Gap | Python library does not parse A1 (0xA1) heartbeat frames |
| OQ-21 | deviceSN8 variant temperature decoding | §5.4.2 | Hypothesis | CA models use different bit positions in B5 temp field. Needs testing across model families (Finding 14a) |
| OQ-22 | Smart Dry mode field | §6.1 | Hypothesis | smartDryValue (body[13] or [19], 7-bit, 30-101%) only in DRY/SMART_DRY mode. Not captured (Finding 14b) |
| OQ-23 | Comfort sleep curve format | §5.2 | Hypothesis | comma-separated hex string; exact encoding undocumented (Finding 14c) |
...
| OQ-25 | MSG 0x0F/0x11 restart trigger | §8.1 | Consistent | body[0]=0x80, body[1]=0x40 triggers dongle restart. Not captured (Finding 13d) |
...
| DA-10 | ~~B0/B1 complete property list~~ | **Resolved** — 53 IDs extracted from Lua Q14 | See §5.5 |
